...
class TiendaAvatar(Base):
    ...
    precio_estrellas = Column(Integer, nullable=False, default=0)
...
class AulaVirtual(Base):
    ...
    id = Column(Uuid(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    anfitrion_id = Column(Uuid(as_uuid=False), ForeignKey("usuarios.id"), nullable=False)
...
class RetoPersonalizado(Base):
    ...
    reto_nivel_id = Column(Integer, ForeignKey("retos_niveles.id"), nullable=False)
    titulo = Column(String, nullable=False)
    estado = Column(Enum(EstadoReto, native_enum=False), nullable=False, default=EstadoReto.borrador)
    tipo_reto = Column(Enum(TipoReto, native_enum=False), nullable=False, default=TipoReto.laberinto)
    parametros_evaluacion = Column(JSON_DOCUMENT, nullable=False)
    recompensa_estrellas = Column(Integer, default=5)
    fecha_creacion = Column(DateTime, default=datetime.utcnow)
...
class ProgresoAula(Base):
    ...
    id = Column(Uuid(as_uuid=False), primary_key=True, default=lambda: str(uuid.uuid4()))
    jugador_id = Column(Uuid(as_uuid=False), ForeignKey("usuarios.id"), nullable=False)
    reto_personalizado_id = Column(Uuid(as_uuid=False), ForeignKey("retos_personalizados.id"), nullable=False)
    completado = Column(Boolean, default=False)
    estrellas_obtenidas = Column(Integer, default=0)
